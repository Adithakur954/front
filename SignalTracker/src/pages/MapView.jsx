...
// Helper functions and child components - use your existing imports
...
// Use your existing components instead of creating new ones
// Remove these imports since they don't exist yet:
// import LayerManager from '../components/map/LayerManager';
// import AdvancedFilters from '../components/map/AdvancedFilters';
// import ServingCellAnalysis from '../components/map/ServingCellAnalysis';
// import ElevationProfile from '../components/map/ElevationProfile';
// import TaskManager from '../components/map/TaskManager';
...
const MapView = () => {
    ...
    // Simplified state for your current structure
    ...
    const activeMetric = useMemo(() => {
        const { lte } = layerVisibility.dtMetrics;
    ...
    useEffect(() => {
        ...
        const fetchInitialSessions = async () => {
            ...
            try {
                ...
                if (validSessionMarkers.length > 0) {
                ...
                toast.error(`Failed to load initial session data: ${error.message}`);
            } finally {
                setLoading(false);
            }
        };
    ...
    useEffect(() => {
        ...
        const markers = networkLogData.map(point => {
            return new window.google.maps.Marker({
                ...
                icon: getMarkerIcon(point[activeMetric.toLowerCase()], activeMetric),
                map: map,
                title: `${activeMetric}: ${point[activeMetric.toLowerCase()] || 'N/A'}`
            });
        });
    ...
    const handleFetchHeatmapData = async () => {
        ...
        try {
            ...
            toast.error(`Failed to fetch heatmap data: ${error.message}`);
        } finally {
            setLoading(false);
        }
    };
        ...
        try {
            ...
            const validLogs = (Array.isArray(logData) ? logData : [])
                ...
                .map(log => ({
                    ...
                    sinr: log.sinr !== null ? Number(log.sinr) : null,
            ...
            toast.error(`Failed to fetch network logs: ${error.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="relative h-screen w-screen">
            ...
            {/* Use your existing MapSidebar instead of new components */}
            <MapSidebar
                sessions={sessions}
                filters={filters}
                        ...
                        <div className="p-2 max-w-sm">
                            ...
                            {selectedItem.type === 'session' ? (
                                <>
                                    ...
                                    <p><strong>Started:</strong> {new Date(selectedItem.data.start_time).toLocaleString()}</p>
